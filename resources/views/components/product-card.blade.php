...
<article class="product-card" data-product-card="{{ $product['id'] }}" data-category-id="{{ $product['category'] }}">
    <div class="product-image-wrap">
        <button class="product-image-button" data-product="{{ $product['id'] }}" aria-label="View {{ $product['name'] }}"><img src="{{ asset('images/figma/'.$product['image']) }}" alt="{{ $product['name'] }}" loading="lazy" width="400" height="290"></button>
    ...
    <p class="product-price"><strong>Rs. {{ number_format($product['price']) }}</strong><del>Rs. {{ number_format($product['original_price']) }}</del></p>
    <div class="product-actions"><button class="button button-navy" data-add="{{ $product['id'] }}"><x-store-icon file="25f90.svg" :size="17" /> Add</button><a class="button button-green" href="https://wa.me/{{ $whatsapp }}?text={{ rawurlencode('Hello, I would like to inquire about '.$product['name'].'. Please confirm price and availability.') }}" target="_blank" rel="noopener"><x-store-icon file="49cd5.svg" :size="16" /> WhatsApp</a></div>
</article>
